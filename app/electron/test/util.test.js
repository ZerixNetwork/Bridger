...
import {detectArchiveType, extractTar, extractZip, isPathInside} from "../src/util.js";
...
describe("archive extraction", () => {
    ...
    it("rejects sibling paths sharing the output prefix", () => {
        assert.equal(isPathInside(outputPath, path.join(dir, "output-evil", "pwned.txt")), false);
        assert.equal(isPathInside(outputPath, path.join(outputPath, "region", "r.0.0.mca")), true);
    });

    it("ignores zip entries targeting a sibling with the output prefix", async () => {
        const filePath = await writeFixture(dir, "prefix-evil.zip", await makeZip([
            {name: "MyWorld/level.dat", data: Buffer.from("LEVELDATA")},
            {name: "MyWorld/../output-evil/zip.txt", data: Buffer.from("PWNED")}
        ]));
        await extractZip(filePath, outputPath, () => {});

        assert.equal(await exists(path.join(dir, "output-evil", "zip.txt")), false);
    });
    ...
    it("ignores tar entries targeting a sibling with the output prefix", async () => {
        const filePath = await writeFixture(dir, "prefix-evil.tar", await makeTar([
            {name: "MyWorld/level.dat", data: Buffer.from("LEVELDATA")},
            {name: "MyWorld/../output-evil/tar.txt", data: Buffer.from("PWNED")}
        ]));
        await extractTar(filePath, outputPath, () => {});

        assert.equal(await exists(path.join(dir, "output-evil", "tar.txt")), false);
    });
});
